...
export default function WhyChoose() {
  ...
  return (
    <section id="why-choose" className="py-32 bg-brand-green-light relative">
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          ...
          <div className="w-full lg:w-1/2">
            ...
            <motion.p
              ...
              className="text-foreground/70 text-lg mb-8 font-medium leading-relaxed"
            >
          ...
          <div className="w-full lg:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {reasons.map((reason, i) => (
              <motion.div
                ...
                className="glass p-6 rounded-2xl hover:bg-white/90 transition-all group border border-foreground/5 hover:border-brand-gold/30 shadow-sm"
              >
                <div className="flex items-start gap-4">
                  <CheckCircle2 className="w-6 h-6 text-brand-gold mt-1 shrink-0 group-hover:scale-110 transition-transform" />
                  <div>
                    <h4 className="text-xl font-bold text-foreground mb-2">{reason.title}</h4>
                    <p className="text-foreground/60 text-sm font-medium leading-relaxed">{reason.desc}</p>
                  </div>
                </div>
              </motion.div>
            ))}
